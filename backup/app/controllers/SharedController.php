...
class SharedController extends BaseController{
	...
	function tb_masterpos_nama_pengusul_option_list(){
		$db = $this->GetModel();
		$sqltext = "SELECT DISTINCT id_karyawan AS value , nama_karyawan AS label FROM tb_karyawan ORDER BY label ASC";
		$queryparams = null;
	...
	function tb_masterpos_kd_unit_option_list(){
		$db = $this->GetModel();
		$sqltext = "SELECT DISTINCT kd_unit AS value , nama_unit AS label FROM tb_unitkerja ORDER BY label ASC";
		$queryparams = null;
		$arr = $db->rawQuery($sqltext, $queryparams);
		return $arr;
	}
